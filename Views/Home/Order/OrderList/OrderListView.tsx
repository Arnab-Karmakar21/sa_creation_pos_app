...
import {ConvertTimeZone} from '../../../../Service/TimeZoneConverter';
const OrderListView = ({
  SearchAction,
  ClearData,
  route,
  ...
  const Search = () => {
    SearchAction({
      ...
      order_status: !!getValues('status_name')
        ...
        : null,
      store_id:
        !!getValues('store_id') && !!+getValues('store_id')
          ? getValues('store_id')
          : null,
    });
  };
  ...
  return (
    <View
      ...
      <View style={{flex: 1.2}}>
        ...
        <Text style={{fontSize: theme.fonts.extremeSmallFont}}>
          #{item.store_name}
        </Text>
      </View>
      <View style={{flex: 1}}>
      ...
      <View style={{flex: 1}}>
        <Text style={{fontSize: theme.fonts.smallFont, fontWeight: '600'}}>
          {ConvertTimeZone(item.scheduled_delivery_date, 'DD/MM/YYYY')}
        </Text>
      </View>
      <View style={{flex: 1}}>
        <Text style={{fontSize: theme.fonts.smallFont, fontWeight: '600'}}>
      ...
      <View style={{flex: 1}}>
        <TouchableOpacity
          style={{flexDirection: 'row'}}
          onPress={() => onViewOrderDetails(item.bag_id)}>
          <Ionicon
            name={'eye'}
            color={theme.colors.primary}
